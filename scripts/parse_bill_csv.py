...
# ── 分类体系统一来源（2026-09-24 收口）──────────────────────
# 分类定义只保留在 backend/app/services/categories.py 一处。
# 本脚本原有的 13 类 + 平台映射 + 关键词下沉已迁到那里，这里只做导入，
# 局部变量名保持不变，以免影响下游代码。
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "backend"))
from app.services import categories as _cat  # noqa: E402
...
# 统一分类体系（支出侧）—— 定义见 backend/app/services/categories.py
_CATEGORIES = _cat.CATEGORIES + (_cat.NEUTRAL_CATEGORY,)   # 含「不计收支」标记
...
_PLATFORM_MAP = _cat.PLATFORM_MAP

# ── 二级：关键词下沉规则（顺序敏感：特定的放前面，避免被宽泛规则抢走）──
_CATEGORY_RULES = _cat.KEYWORD_RULES

# ── 性质：刚性支出词表（不可压缩 / 有法定义务 / 按月固定）──
_RIGID_WORDS = _cat.RIGID_WORDS

# ── 归属：经营成本词表（与经营直接相关、可能取得票据）──
_BUSINESS_WORDS = _cat.BUSINESS_WORDS

# 「不计收支」类值：这些不是收入也不是支出，纳入统计会虚高
_NEUTRAL_VALUES = _cat.NEUTRAL_VALUES
